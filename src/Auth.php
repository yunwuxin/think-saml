...
class Auth
{
    ...
    public function acs(IdentityProvider $idp, Request $request)
    {
        $xml = base64_decode($request->param('SAMLResponse'));

        $response = new Response($xml, $this->sp->getKey(), $idp->getData(), $this->security);
